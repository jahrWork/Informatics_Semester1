...
\end{lstlisting}
	...
	Implementad los siguientes 6 apartados secuenciales 
	utilizando \textbf{exclusivamente funciones de NumPy}. 
	
	\subsection*{1. Resolución de un sistema lineal (Calibración de la Antena)}
	Las estaciones terrestres de Starlink utilizan antenas 
	de puesta en fase (phased-array). 
	Durante la recepción, 
	existe un acoplamiento electromagnético 
	entre tres sectores principales. 
	El retardo real de propagación de la señal 
	$\mathbf{x} = [x_1, x_2, x_3]^T$ se relaciona 
	con el retardo aparente medido $\mathbf{b_{cal}}$ 
	mediante la matriz de interferencia $\mathbf{A_{cal}}$ 
	según la ecuación: $\mathbf{A_{cal}} \mathbf{x} = \mathbf{b_{cal}}$.
	
	\begin{itemize}
		\item \textbf{Se pide:} Resolver el sistema lineal 
		algebraico para hallar el vector de retardos verdaderos 
		$\mathbf{x}$ empleando las rutinas de álgebra lineal.
	\end{itemize}
	...
	\begin{itemize}
		...
		\item El código debe seguir las normas 
		de una metode estilo con un código modularizado 
		(uso de funciones \texttt{def} para separar la matemática 
		de la interfaz gráfica).
		\item Se valorará profundamente 
		la interpretación física de los resultados 
		impresa tanto en los comentarios del código como en la propia interfaz visual.
	\end{itemize}